...
class WordCloud {
    ...
    this.run(words, list => {
      list.forEach((item, index) => {
        ...
        if (exist) {
          ...
          inner.id = "simpleWordCloudWordItemInner" + index
          inner.style.cssText = `
            white-space: nowrap;
          `
        ...
        setTimeout(() => {
          ...
          inner.style.border = '1px solid #3877F2'
          inner.style.padding = '6px 10px 4px 6px'
          inner.style.opacity = 1- Math.floor(Math.random() * (10 - 5))/10
          inner.style.boxShadow = '0px 0px 10px 0px #008CB299 inset'
          inner.style.background = '#0A3C57'
  ...
  // 选择一个想要的
  select() {
    // console.log(this.wordItemElList)
    let maxNum = this.wordItemElList.length-1
    let cyclesNum = this.randomNum(15, 30)
    for (let i = 0; i < cyclesNum; i++) {
      
      setTimeout(() => {
        if(this.historyId != -1){
          let dom = document.getElementById("simpleWordCloudWordItemInner"+this.historyId)
          dom.style.color = this.historyColor
          dom.style.textShadow = "none"
      }
        let id = this.randomNum(0, maxNum)
        let dom = document.getElementById("simpleWordCloudWordItemInner"+id)
        // console.log("========",dom);
        
        this.historyId = id
        this.historyColor = dom.style.color

        dom.style.color = 'red'
        dom.style.textShadow = "2px 2px 5px red"
      }, i*100);
    }
  }
